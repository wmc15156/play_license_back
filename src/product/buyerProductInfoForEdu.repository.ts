...
export class BuyerProductInfoForEduRepository extends Repository<BuyerProductInfoForEdu> {
  ...
  async updateData(
    ...
    data.requiredMaterials =
      Array.isArray(updateProductDto.requiredMaterials) &&
      updateProductDto.requiredMaterials.join(',');
  ...
  async getInquiryForEducation(
    userId: number,
    productId: number,
  ): Promise<BuyerProductInfoForEduRepository> {
    return await this.createQueryBuilder('product')
      .leftJoinAndSelect('product.user', 'user')
      .leftJoinAndSelect('product.product', 'providerProduct')
      .where('user.userId = :userId', { userId })
      .andWhere('product.productId = :productId', { productId })
      .select(['product', 'providerProduct.title'])
